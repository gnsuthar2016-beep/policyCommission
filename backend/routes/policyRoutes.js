...
router.post('/api/import/policies', (req, res, next) => {
  ...
  try {
    ...
    const parseDateValue = (field, value, fallback, errors) => {
      const val = value ? String(value).trim() : '';
      if (!val) {
        return fallback;
      }
    ...
    for (const batch of batches) {
      ...
      try {
        const createdItems = await Policy.bulkCreate(batchData, { validate: false, individualHooks: false });
...
router.post('/api/policy', async (req, res) => {
  try {
    ...
    const policy = await Policy.create({
      ...
      ncbAmount: policyData.ncbAmount != null ? policyData.ncbAmount : null,
